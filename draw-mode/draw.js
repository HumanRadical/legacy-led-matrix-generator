...
const createDrawGrid = (frame) => {
    ...
    const colorValues = sanitizeColorArrayIntoHex(frame.value)
...
const switchFrameLeft = (event) => {
    ...
    if (frameBoxes[currentFrameIndex - 1]) {
        currentFrameIndex--
        createDrawGrid(frameBoxes[currentFrameIndex])
    }
}
...
window.onload = createDrawGrid(frameBoxes[currentFrameIndex])